...
def mogp():
    ...
    pyemu.os_utils.start_workers("template","./pest/pestpp-mou","./pest/bm4_gp.pst",num_workers=64,worker_root=".",master_dir="./mogp_"+str(next_inner_index))  
...
def restart_prep():
    ...
    obs_restart.to_csv(os.path.join(path_in,"mogp_bm4.obs_pop.csv"), index=False)
    dv_restart.to_csv(os.path.join(path_in,"mogp_bm4.dv_pop.csv"), index=False)
...
def start_prep():
    ...
    curr_obs.to_csv(os.path.join(path_in,"mogp_bm4.obs_pop.csv"), index=False)
    ...
    curr_dv.to_csv(os.path.join(path_in,"mogp_bm4.dv_pop.csv"), index=False)
...
def resample():      
    ...
    while ((n_infill < max_infill) & ( ppd_front <= maxppf)):
        inner_pareto = inner_pareto_summary[inner_pareto_summary['nsga2_front'] == ppd_front]
